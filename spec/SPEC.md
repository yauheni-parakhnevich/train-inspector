...
| `--fast` | off | Bilinear resampling; disables cross-dissolve blending — single-frame wide strips (§8). |

## 6. Non-Functional Requirements

- **NFR-1 Performance:** Process 1080p/60fps **H.264** footage at ≥ 2× real-time on an Apple Silicon laptop (a 60 s clip in ≤ 30 s) (codec pinned per review N12). Memory bounded: mosaic assembled incrementally in chunks; never holds all decoded frames. For long recordings with a short pass (e.g., 20-min clip, 1-min train), pass 1 may use a strided coarse pre-scan (every K-th frame) to bracket candidate motion regions before dense estimation; this is an internal optimization, not user-visible (review Q1). The ≥ 2× real-time target is measured on the `--fast` path; the default cross-dissolve path trades a little speed for seam-free output (one extra warp per frame pair). Trial (4K freight_crossing.mp4): `--fast` 7s, default cross-dissolve 8s.
...
Options:
  ...
  --fast                      Faster (bilinear, no cross-dissolve blending); wide single-frame strips
...
- **Motion Estimator** — two-pass design:
  ...
  - **Pass-2 cross-dissolve (default):** each strip is a motion-compensated
    temporal cross-dissolve between the two frames bounding the interval,
    aligned by the pass-1 displacement (`composite.py`, `_crossfade`).
    Consecutive strips meet at a shared frame, so the per-frame boundary
    (exposure / motion blur / sub-pixel) is smoothed away instead of appearing
    as a vertical seam; strip width still comes from the pass-1 smoothed dx via
    the carry accumulator (§10.3). Dense optical flow for *per-row* perspective
    correction was prototyped and rejected: it is unreliable at the
    displacements real footage exhibits (tens of px/frame — DIS returns
    near-zero or wrong-sign motion there and injects noise rather than helping;
    measured −26% vertical-line energy with the global cross-dissolve vs a
    *regression* with per-row flow on a 50 px/frame clip). The former
    phase-correlation refinement step (review M6) is also removed. `--fast`
    selects the single-frame wide-strip path for the whole pass.
...
- **Dropped-frame gaps:** when frames are dropped (the inter-frame `dt` spikes), velocity smoothing scales `dx` up to fill the gap (review M4), but a single decoded frame cannot represent a multi-frame temporal sweep — the velocity-extrapolated wide strip re-samples surface the neighbouring strips already cover, leaving a **ghost** (a faint duplicate of the train near the gap). Since the dropped frames are unrecoverable, a strip whose `dt` exceeds `DROP_DT_FACTOR × median dt` is capped at `MAX_STRIP_FACTOR × median dx` (`pipeline._capped_dx`) **and composited as a single-frame wide strip, not a cross-dissolve** — capping the displacement below the true gap motion would mis-align the two blended frames (they no longer show the same surface) and re-introduce the ghost. The gap becomes a small, locally-invisible compression instead of a visible ghost, and a warning is logged. Legitimately fast frames (normal `dt`) are never capped.

## 11. Acceptance Criteria

1. **Constant speed:** synthetic fixture (textured rectangle "train", lossless FFV1, generated in-repo) crossing at constant speed → output width equals train length in pixels ±1%, and pixelwise comparison against the **analytically known source texture** yields SSIM ≥ 0.98 (measured on the `--fast` wide-strip path — the pure-geometry guarantee; the default cross-dissolve path is held to SSIM ≥ 0.97 with no seam-energy regression). No stored golden images (review M10).
