...
- **Glitched timestamps:** real footage can report a spurious large inter-frame `dt` (a VFR / container PTS hiccup) while the train actually moved a *normal* amount between the two decoded frames. The velocity-domain smoothing (review M4) would treat the correct, confidently-tracked raw displacement (say 46 px) as a velocity outlier and replace it with the velocity prediction scaled by the bad `dt` — inflating it into a wide strip (say 167 px) that re-samples surface the neighbouring strips already cover, leaving a **ghost** (a faint duplicate near the train head). Guard (`motion.smooth`, `RAW_TRUST_CONF` / `DT_ANOMALY_FACTOR`): a frame rejected as a velocity outlier whose `dt` is anomalously large **and** whose LK confidence is high keeps its raw displacement — LK measured the true inter-frame motion, so it is trusted over the extrapolation. Genuine dropped frames fail LK (low confidence) and correctly keep the velocity prediction.
